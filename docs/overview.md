...
| **DRA Driver** | Publishes devices as schedulable resources via Kubernetes Dynamic Resource Allocation. |
| **Multi-Node Scheduling** | Groups and wires up multi-node jobs. |
...
The remaining components extend this. The Fabric Manager resolves topology that
the DRA Driver and Telemetry consume, and the DRA Driver makes devices available
to workloads.
