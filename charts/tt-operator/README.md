...
| oci://ghcr.io/tenstorrent/helm | tt-telemetry | 0.1.3 |
...
| tt-fabric-manager.controller.affinity | object | `{"nodeAffinity":{"preferredDuringSchedulingIgnoredDuringExecution":[{"preference":{"matchExpressions":[{"key":"feature.node.kubernetes.io/pci-1200_1e52.present","operator":"DoesNotExist"},{"key":"tenstorrent.com/has-tt","operator":"NotIn","values":["true"]}]},"weight":100}]}}` | TTFM controller affinity. Prefers nodes with no Tenstorrent device (i.e. infra hosts) so the controller stays off device hosts like Galaxy, while still scheduling in all-device clusters. |
...
| tt-telemetry.image.tag | string | `"0.1.3"` | tt-telemetry image tag. Keep in lockstep with the tt-telemetry subchart version in Chart.yaml. |
